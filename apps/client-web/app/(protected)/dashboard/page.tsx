      ...
      <div className="container py-8">
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2">
            ...
            {upcomingAppointments.length > 0 ? <div className="space-y-4">{upcomingAppointments.slice(0, 3).map((appointment) => { const status = getAppointmentStatusLabel(appointment.status); return <Card key={appointment.id} className="p-6"><div className="mb-4 flex items-start justify-between gap-4"><div><h3 className="mb-1 font-medium">{appointment.petName}</h3><p className="mb-2 text-sm text-muted-foreground">{appointment.itemLabels.join(", ")}</p><div className="flex items-center gap-2 text-sm"><CalendarIcon className="h-4 w-4 text-primary" /><span>{formatDateLong(appointment.startAt)}, {formatTime(appointment.startAt)}</span></div></div><Badge variant={status.tone}>{status.label}</Badge></div><div className="flex items-center justify-between border-t border-border pt-4"><span className="text-sm text-muted-foreground">{appointment.status}</span><Link href={`/dashboard/appointments/${appointment.id}`}><Button variant="outline" size="sm">Деталі</Button></Link></div></Card>; })}</div> : <Card className="p-12 text-center"><CalendarIcon className="mx-auto mb-4 h-16 w-16 text-muted-foreground" /><h3 className="mb-2 font-medium">Немає майбутніх записів</h3><p className="mb-4 text-sm text-muted-foreground">Запишіться на зручний час прямо зараз</p><Link href="/booking/services"><Button>Записатися</Button></Link></Card>}
          </div>
          <div>
