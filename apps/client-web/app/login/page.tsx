...
import { createClientBrowserSessionRequest, storeSession } from "@/lib/auth";
...
export default function LoginPage() {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    setError(null);
    try {
      const response = await publicApiRequest<ClientLoginResponse>("/api/client/auth/login", {
      ...
      const hasActiveBooking = typeof window !== "undefined" && window.sessionStorage.getItem("tailbook.client.booking.v2") !== null;
      router.replace(hasActiveBooking ? "/booking/services" : "/dashboard");
    } catch (err) {
          ...
          <form onSubmit={onSubmit} className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between gap-4">
                ...
                <Link href="/forgot-password" className="text-sm text-primary underline">Забули?</Link>
              </div>
              <Input id="password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="••••••••" className="mt-2" />
            </div>
