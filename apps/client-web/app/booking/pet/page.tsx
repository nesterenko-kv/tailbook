...
function BookingPetContent() {
  ...
  function handleContinue() {
    ...
    const nameResult = validateInput({ value: petName, required: true, label: "ім'я вихованця" });
    if (nameResult) errors.petName = nameResult;
    if (!animalTypeId) {
      errors.animalType = { message: "Вкажіть тип вихованця." };
    }
        ...
        <div className="mx-auto max-w-2xl">
          ...
          <div className="space-y-6">
            {mode === "saved" ? (
              ...
              <>
                <div aria-invalid={fieldErrors.animalType ? "true" : undefined} aria-describedby={fieldErrors.animalType ? "animalType-error" : undefined}>
                  <Label className="mb-3 block">Тип вихованця *</Label>
                  <div className="grid grid-cols-2 gap-4">
                    {(catalog?.animalTypes ?? []).map((item) => (
                      <button key={item.id} type="button" onClick={() => { setAnimalTypeId(item.id); setBreedId(""); setCoatTypeId(""); setSizeCategoryId(""); setFieldErrors((prev) => ({ ...prev, animalType: undefined })); }} className={animalTypeId === item.id ? "rounded-lg border-2 border-primary bg-accent p-4 text-center" : "rounded-lg border-2 border-border p-4 text-center"}>
                        <span className="mb-2 block text-2xl">{item.code === "DOG" ? "🐕" : "🐈"}</span>
                        <span className="font-medium">{item.name}</span>
                      </button>
                    ))}
                  </div>
                  {fieldErrors.animalType ? <p id="animalType-error" className="mt-1 text-sm text-red-600" role="alert">{fieldErrors.animalType.message}</p> : null}
                </div>

                <div aria-invalid={fieldErrors.petName ? "true" : undefined} aria-describedby={fieldErrors.petName ? "petName-error" : undefined}>
                  <Label htmlFor="petName">Ім'я вихованця *</Label>
                  <Input id="petName" value={petName} onChange={(event) => { setPetName(event.target.value); setFieldErrors((prev) => ({ ...prev, petName: undefined })); }} placeholder="Наприклад, Джек" className="mt-2" />
                  {fieldErrors.petName ? <p id="petName-error" className="mt-1 text-sm text-red-600" role="alert">{fieldErrors.petName.message}</p> : null}
                </div>

                <div>
                  <Label htmlFor="breed">Порода</Label>
                  <NativeSelect id="breed" value={breedId} onChange={(event) => { setBreedId(event.target.value); setFieldErrors((prev) => ({ ...prev, breed: undefined })); }} className="mt-2">
                ...
                <div>
                  ...
                  <NativeSelect id="coatType" value={coatTypeId} onChange={(event) => { setCoatTypeId(event.target.value); setFieldErrors((prev) => ({ ...prev, coatType: undefined })); }} className="mt-2">
                ...
                <div>
                  ...
                  <NativeSelect id="sizeCategory" value={sizeCategoryId} onChange={(event) => { setSizeCategoryId(event.target.value); setFieldErrors((prev) => ({ ...prev, sizeCategory: undefined })); }} className="mt-2">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div><Label htmlFor="weightKg">Вага (необов'язково)</Label><Input id="weightKg" value={weightKg} onChange={(event) => { setWeightKg(event.target.value); setFieldErrors((prev) => ({ ...prev, weightKg: undefined })); }} placeholder="Наприклад, 15" className="mt-2" /></div>
