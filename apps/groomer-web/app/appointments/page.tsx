...
import { clearSession, getStoredEmail, getStoredAccessToken } from "@/lib/auth";
...
export default function AppointmentsPage() {
    ...
    return (
        <main className="mx-auto flex min-h-screen max-w-6xl flex-col gap-8 px-6 py-10">
            <header className="flex flex-col gap-4 rounded-3xl border border-slate-800 bg-slate-900/70 p-6 md:flex-row md:items-end md:justify-between">
                <div>
                    ...
                    <p className="mt-2 text-sm text-slate-300">Signed in as {email ?? "groomer"}. This view is privacy-safe and excludes CRM contacts by design.</p>
                </div>
                <div className="flex gap-3">
                    <Link href="/login" className="rounded-2xl border border-slate-700 px-4 py-2 text-sm text-slate-200">Switch account</Link>
                    <button onClick={() => { clearSession(); router.push("/login"); }} className="rounded-2xl bg-slate-800 px-4 py-2 text-sm text-slate-100">Sign out</button>
                </div>
            </header>
