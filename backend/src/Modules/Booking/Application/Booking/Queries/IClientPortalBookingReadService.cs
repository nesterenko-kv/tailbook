...
public interface IClientPortalBookingReadService
{
    ...
    Task<ErrorOr<QuotePreviewView>> PreviewMyQuoteAsync(ClientPortalActor actor, PreviewQuoteQuery command, CancellationToken cancellationToken);
    Task<IReadOnlyCollection<ClientAppointmentSummaryView>> ListMyAppointmentsAsync(Guid clientId, DateTimeOffset? from, CancellationToken cancellationToken);
