...
public sealed class DevelopmentDemoSalonSeeder(IHostEnvironment environment, TimeProvider timeProvider) : IDataSeeder
{
    ...
    private static async Task EnsureGroomersAsync(AppDbContext dbContext, DateTimeOffset utcNow, CancellationToken cancellationToken)
    {
    ...
    private static async Task<Dictionary<string, ProcedureCatalogItem>> EnsureProceduresAsync(AppDbContext dbContext, DateTimeOffset utcNow, CancellationToken cancellationToken)
    {
        var seeds = new[]
        {
    ...
    private static async Task<Dictionary<string, CommercialOffer>> EnsureOffersAsync(AppDbContext dbContext, DateTimeOffset utcNow, CancellationToken cancellationToken)
    {
        var seeds = new[]
        {
        ...
        IReadOnlyDictionary<string, SizeCategory> SizeCategories)
    {
        public ResolvedCondition Resolve(string? animalTypeCode, string? breedCode, string? breedGroupCode, string? coatTypeCode, string? sizeCategoryCode)
        {
            ...
            var specificityScore = new[] { animalTypeId, breedId, breedGroupId, coatTypeId, sizeCategoryId }.Count(x => x.HasValue);
