...
public sealed class PetsUseCases(
    ...
    ILogger<PetsUseCases> logger,
    TimeProvider timeProvider) : IPetsReadService
{
    private static readonly JsonSerializerOptions CacheJsonOptions = new(JsonSerializerDefaults.Web);
    private static readonly DistributedCacheEntryOptions PetCatalogCacheOptions = new()
    {
    ...
    {
        var cachedCatalog = await TryReadPetCatalogCacheAsync(cancellationToken);
        ...
        await TryWritePetCatalogCacheAsync(catalog, cancellationToken);
        return catalog;
    }

    private async Task<PetCatalogView> LoadCatalogAsync(CancellationToken cancellationToken)
    {
    ...
    private async Task<PetCatalogView?> TryReadPetCatalogCacheAsync(CancellationToken cancellationToken)
    {
        try
        {
            var payload = await distributedCache.GetStringAsync(PetCatalogCacheKeys.Catalog, cancellationToken);
            if (string.IsNullOrWhiteSpace(payload))
            {
                return null;
            }

            var cached = JsonSerializer.Deserialize<CachedPetCatalogView>(payload, CacheJsonOptions);
            return cached?.ToView();
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            logger.PetCatalogCacheReadFailed(ex);
            return null;
        }
    }

    private async Task TryWritePetCatalogCacheAsync(PetCatalogView catalog, CancellationToken cancellationToken)
    {
        try
        {
            var payload = JsonSerializer.Serialize(CachedPetCatalogView.FromView(catalog), CacheJsonOptions);
            await distributedCache.SetStringAsync(PetCatalogCacheKeys.Catalog, payload, PetCatalogCacheOptions, cancellationToken);
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            logger.PetCatalogCacheWriteFailed(ex);
        }
    }
    ...
    public async Task<ErrorOr<PetDetailView>> UpdatePetAsync(Guid petId, UpdatePetInput command, CancellationToken cancellationToken)
    {
    ...
    private async Task<PetDetailView?> LoadPetProjectionAsync(Guid petId, CancellationToken cancellationToken)
    {
        ...
        var photos = await dbContext.Set<PetPhoto>()
            ...
            .Select(x => new PetPhotoView(x.Id, x.StorageKey, x.FileName, x.ContentType, x.IsPrimary, x.SortOrder, x.CreatedAt))
            .ToListAsync(cancellationToken);
    ...
    private sealed record CachedPetCatalogView(
        AnimalTypeView[] AnimalTypes,
...
internal static partial class PetsUseCasesLogMessages
{
    [LoggerMessage(
        EventId = 5001,
        Level = LogLevel.Warning,
        Message = "Pet catalog cache read failed; loading from the database.")]
    public static partial void PetCatalogCacheReadFailed(this ILogger logger, Exception exception);

    [LoggerMessage(
        EventId = 5002,
        Level = LogLevel.Warning,
        Message = "Pet catalog cache write failed; continuing without caching.")]
    public static partial void PetCatalogCacheWriteFailed(this ILogger logger, Exception exception);
}
