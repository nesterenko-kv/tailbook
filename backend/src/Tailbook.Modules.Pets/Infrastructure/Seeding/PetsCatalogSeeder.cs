...
namespace Tailbook.Modules.Pets.Infrastructure.Seeding;
...
public sealed class PetsCatalogSeeder : IDataSeeder
{
    ...
    public async Task SeedAsync(AppDbContext dbContext, IServiceProvider serviceProvider, CancellationToken cancellationToken)
    {
        ...
        if (distributedCache is not null)
        {
            try
            {
                await distributedCache.RemoveAsync(PetCatalogCacheKeys.Catalog, cancellationToken);
            }
            catch (Exception ex) when (ex is not OperationCanceledException)
            {
                serviceProvider.GetService<ILogger<PetsCatalogSeeder>>()?.PetCatalogCacheInvalidationFailed(ex);
            }
        }
    }
...
internal static partial class PetsCatalogSeederLogMessages
{
    [LoggerMessage(
        EventId = 5003,
        Level = LogLevel.Warning,
        Message = "Pet catalog cache invalidation failed after seeding; continuing startup.")]
    public static partial void PetCatalogCacheInvalidationFailed(this ILogger logger, Exception exception);
}
