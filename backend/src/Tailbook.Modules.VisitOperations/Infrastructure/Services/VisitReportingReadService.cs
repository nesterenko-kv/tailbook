...
{
    public async Task<IReadOnlyCollection<VisitEstimateAccuracyRow>> ListEstimateAccuracyRowsAsync(DateTimeOffset? from, DateTimeOffset? to, CancellationToken cancellationToken)
    {
        var visitsQuery = dbContext.Set<Visit>()
            .Where(x => x.Status == VisitStatusCodes.Closed && x.ClosedAt != null);

        if (from.HasValue) visitsQuery = visitsQuery.Where(x => x.ClosedAt >= from.Value);
        if (to.HasValue) visitsQuery = visitsQuery.Where(x => x.ClosedAt <= to.Value);

        var visits = await visitsQuery.OrderByDescending(x => x.ClosedAt).ToListAsync(cancellationToken);
    ...
    public async Task<IReadOnlyCollection<VisitPackagePerformanceRow>> ListPackagePerformanceRowsAsync(DateTimeOffset? from, DateTimeOffset? to, CancellationToken cancellationToken)
    {
        var visitsQuery = dbContext.Set<Visit>()
            .Where(x => x.Status == VisitStatusCodes.Closed && x.ClosedAt != null);
