...
public interface IVisitReadService
{
    ...
    Task<ErrorOr<PagedResult<VisitListItemView>>> ListVisitsAsync(string? search, string? status, DateTimeOffset? from, DateTimeOffset? to, Guid? groomerId, Guid? appointmentId, int page, int pageSize, CancellationToken cancellationToken);
}
