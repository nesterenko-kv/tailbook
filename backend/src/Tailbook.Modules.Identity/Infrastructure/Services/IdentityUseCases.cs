...
public sealed class IdentityUseCases(AppDbContext dbContext, PasswordHasher passwordHasher, IAuditTrailService auditTrailService, TimeProvider timeProvider) : IIdentityReadService
{
    ...
    public async Task<ErrorOr<UserDetailView>> GetUserAsync(Guid userId, CancellationToken cancellationToken)
    {
    ...
    public async Task<ErrorOr<UserDetailView>> CreateUserAsync(string email, string displayName, string password, IReadOnlyCollection<string> roleCodes, Guid? assignedByUserId, CancellationToken cancellationToken)
    {
        ...
        if (exists)
        {
            return IdentityErrors.UserEmailExists(email);
        }
