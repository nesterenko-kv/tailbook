...
public sealed class StaffUseCases(
    ...
    TimeProvider timeProvider) : IStaffReadService
{
    ...
    public async Task<ErrorOr<GroomerCapabilityView>> AddCapabilityAsync(AddGroomerCapabilityInput command, CancellationToken cancellationToken)
    {
    ...
    public async Task<ErrorOr<TimeBlockView>> AddTimeBlockAsync(Guid groomerId, DateTimeOffset startAt, DateTimeOffset endAt, string reasonCode, string? notes, CancellationToken cancellationToken)
    {
        var groomerExists = await dbContext.Set<Groomer>().AnyAsync(x => x.Id == groomerId, cancellationToken);
        if (!groomerExists)
        {
        ...
        if (endAt <= startAt)
        {
            return Error.Validation("Staff.InvalidTimeBlockRange", "Time block endAt must be later than startAt.");
        }
    ...
    public async Task<ErrorOr<GroomerScheduleView>> GetScheduleAsync(Guid groomerId, DateTimeOffset from, DateTimeOffset to, CancellationToken cancellationToken)
    {
        var groomer = await dbContext.Set<Groomer>().SingleOrDefaultAsync(x => x.Id == groomerId, cancellationToken);
        if (groomer is null)
        {
        ...
        var windows = BuildAvailabilityWindows(from, to, schedules, timeBlocks);
    ...
    private IReadOnlyCollection<AvailabilityWindowView> BuildAvailabilityWindows(DateTimeOffset from, DateTimeOffset to, IReadOnlyCollection<WorkingSchedule> schedules, IReadOnlyCollection<TimeBlock> timeBlocks)
    {
        ...
        while (currentLocalDate <= endLocalDate)
        {
            ...
            if (schedule is not null)
            {
                ...
                var windowEnd = TimeZoneInfo.ConvertTimeToUtc(localEnd, timeZone);
    ...
    private static void AppendAvailabilitySegments(List<AvailabilityWindowView> windows, DateTimeOffset windowStart, DateTimeOffset windowEnd, IReadOnlyCollection<TimeBlock> timeBlocks)
    {
        ...
        foreach (var block in overlappingBlocks)
        {
            ...
            {
                windows.Add(new AvailabilityWindowView(cursor, block.StartAt));
            }
        ...
        if (cursor < windowEnd)
        {
            windows.Add(new AvailabilityWindowView(cursor, windowEnd));
        }
    }
