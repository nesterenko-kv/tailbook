...
## Catalog offer staged preview
- Stage catalog-offer CSVs with `POST /api/admin/catalog/imports/offers/preview` before any mutation workflow is enabled.
- Required headers are `ExternalId`, `Code`, `DisplayName`, `PriceAmount`, `ServiceMinutes`, and `ReservedMinutes`.
- Review recent staged batches with `GET /api/admin/catalog/imports/offers`.
- Download row-level validation failures with `GET /api/admin/catalog/imports/offers/{batchId}/errors.csv`.
- Stage 1 does not commit catalog offers; operators must wait for the commit workflow before using staged rows to mutate catalog data.
